...
public class LoginFragment_JvHe extends LoginBaseFragment{
     ...
     * 登陆店铺编号
     **/
    @BindView(R.id.et_userLoginShopID)
    ...
    public View initView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.fragment_login_jvhe, container, false);
    ...
    @OnClick({R.id.btn_login, R.id.tv_register, R.id.tv_forgotPassword
            , R.id.iv_clearUserNumber, R.id.iv_clearUserPassword})
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_login:
                login_JvHe(et_userNumber.getText().toString().trim()
                        ,et_userPassword.getText().toString().trim()
                        ,et_userLoginShopID.getText().toString().trim());
                break;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        LogUtil.LogShitou("Fragment的onActivityResult方法被执行");
