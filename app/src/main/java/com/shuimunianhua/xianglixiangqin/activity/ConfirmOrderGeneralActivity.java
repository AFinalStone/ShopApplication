...
import com.shuimunianhua.xianglixiangqin.model.ShoppingCartPostModel;
...
public class ConfirmOrderGeneralActivity extends MyBaseActivity implements OnConnectServerStateListener<Integer> {
    ...
    @Bind(R.id.tv_totalOrdersPrices)
    ...
    public void initView() {
        setContentView(R.layout.activity_confirm_order_activity_ex);
        ...
        iv_managerReceiverAddress.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent mIntent = new Intent(mContext, ReceiveAddressManageActivity.class);
                startActivityForResult(mIntent, RequestCode_ShoppingAddressManageActivity);
            }
        });
        listView.addHeaderView(listViewHeader);

        List<ShoppingCartParentGoodsModel> listTempData = (List<ShoppingCartParentGoodsModel>) getIntent()
                .getSerializableExtra(InformationCodeUtil.IntentConfirmOrderGeneralActivityShopCartList);
    ...
    private void toPayMethodSelectActivity() {
        ...
        if(listModel.size() == 0){
            ToastUtils.show(mContext,"请至少选择一个订单");
            return;
        }
        ...
        try {
            ...
            if(IfContainUnCreditOrderItem){
                type = ParamPayMethodSelectModel.TypeConfirmOrderGeneralCanNotPayCredit;
            }else{
                type = ParamPayMethodSelectModel.TypeConfirmOrderGeneralCanPayCredit;
            }
            ParamPayMethodSelectModel paramSport = new ParamPayMethodSelectModel(listModel.size(),totalOrdersPrices,
                    gson.toJson(listModel),strCustomerName,strCustomerReceiverAddress
                    ,strCustomerPhone,type);
    ...
    private void getCurrentOrderTotalPrice() {
        ...
        totalOrdersPrices = orderPrices-totalPrivilegeFlyCoin;
