...
public class WXShopAddGoods extends MyBaseActivity implements OnClickListener,
     ...
     * 特价商品设置
     **/
    @Bind(R.id.linearLayout_settingGoods)
...
//			if(firClassID == -1){
//				ToastUtils.show(mContext, "请为商品分类1选择商品分类");
//				return;
//			}
//			if(secClassID == -1){
//				ToastUtils.show(mContext, "请为商品分类2选择商品分类");
//				return;
//			}
    ...
    public void connectServiceSuccessful(String returnString,
        ...
        if (InformationCodeUtil.methodNameSetEcShopRecClass.equals(methodName)) {
            JSONResultMsgModel msgModel = gson.fromJson(returnString,
                    JSONResultMsgModel.class);
            ToastUtils.show(mContext, msgModel.getMsg());
        }
    ...
    public void connectServiceFailed(String methodName, Integer state,
        ...
        if (InformationCodeUtil.methodNameGetEcShopRecClass.equals(methodName)) {
            ToastUtils.show(mContext, "网络异常，微店推荐栏目数据获取失败");
            return;
        }

        if (InformationCodeUtil.methodNameSetEcShopRecClass.equals(methodName)) {
            ToastUtils.show(mContext, "网络异常，微店推荐栏目设置失败");
            return;
        }
