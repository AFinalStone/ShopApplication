...
import com.shuimunianhua.xianglixiangqin.activity.ConfirmOrderGeneralActivity;
...
public class MainShoppingCartFragment extends MyBaseFragment<MyBaseActivity> implements SwipeRefreshLayout.OnRefreshListener, OnClickListener, OnConnectServerStateListener<Integer> {
	...
	/**购物车ListView适配器**/
	private  ShoppingCartAdpter mShoppingCartAdpter;
	...
	private void initView() {
		...
		mShoppingCartAdpter = new ShoppingCartAdpter(mActivity, listData);
		listView_ShoppingCart.setAdapter(mShoppingCartAdpter);
	}
		...
		switch (v.getId()){
			case R.id.tv_titleLeft:
				if(listData != null && listData.size() > 0){
					...
					mShoppingCartAdpter.notifyDataSetChanged();
				}
				break;
...
//			EventBus.getDefault().postSticky(new ConfirmOrderActivityEvent<List<ShoppingCartParentGoodsModel>>(list));
			...
			intent.putExtra(InformationCodeUtil.IntentConfirmOrderGeneralActivityShopCartList, (Serializable) list);
			mActivity.startActivity(intent);
			mActivity.overridePendingTransition(R.anim.in_from_top, R.anim.out_to_bottom);
		}
	}

	/**获取购物车商品**/
	private void getShopCartData(){
		final String methodName = InformationCodeUtil.methodNameLoadShopCart;
		...
		if (InformationCodeUtil.methodNameChangeProductNum == methodName
			...
			try {
				...
				if(mJsonResult.getSign() == 1){
					ToastUtils.show(mActivity,""+mJsonResult.getMsg());
	...
	public void connectServiceFailed(String methodName, Integer state, boolean whetherRefresh) {
		...
		swipeRefreshLayout.setRefreshing(false);
		if(InformationCodeUtil.methodNameLoadShopCart == methodName){
			ToastUtils.show(mActivity,"网络异常，获取购物车商品失败");
			listData.clear();
			listData.addAll(listCacheData);
			return;
		}
		...
		if (InformationCodeUtil.methodNameBatchDelShopCart == methodName) {
			ToastUtils.show(mActivity,"网络异常，商品删除失败");
			listData.clear();
			listData.addAll(listCacheData);
			return;
		}
		...
		if (InformationCodeUtil.methodNameChangeProductNum == methodName) {
			ToastUtils.show(mActivity,"网络异常，购买数量修改失败");
			listData.clear();
			listData.addAll(listCacheData);
		}
	 ...
	 * 2016-2-17 15:44:41
	 */
	public class ShoppingCartAdpter extends MyBaseAdapter<ShoppingCartParentGoodsModel> {
		
		public ShoppingCartAdpter(Context mContext,
				...
				viewHolderChild.iv_select.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						...
						for (i = 0; i<listChilds.size(); i++){
							if(listChilds.get(i).IfSelect != mChildGoodsModel.IfSelect){
								break;
							}
						}
						if( i == listChilds.size()){
							mParentGoodsModel.IfSelect = mChildGoodsModel.IfSelect;
							mShoppingCartAdpter.notifyDataSetChanged();
						}else{
