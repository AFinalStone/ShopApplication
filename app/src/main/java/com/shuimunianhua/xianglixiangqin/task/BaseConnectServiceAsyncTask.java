 ...
 * 2016年5月12日 10:49:51
 */
public abstract class BaseConnectServiceAsyncTask extends AsyncTask<SoapObject, SoapObject, String> {
	...
	protected void onCancelled(String result) {
		connectServerStateListener.connectServiceCancelled(result, methodName, status, whetherRefresh);
	}
	...
	protected void onPostExecute(String result) {
		if(result == null){
			//请求失败
			connectServerStateListener.connectServiceFailed(methodName,status,whetherRefresh);
		}else{
			//请求成功
			connectServerStateListener.connectServiceSuccessful(result, methodName, status, whetherRefresh);
		}
	}
}
